  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full rounded-lg border border-stone-200 bg-white px-4 py-2.5 text-sm text-stone-600 hover:bg-stone-50 focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 transition-colors"
      >
        この局面について質問する...
      </button>
    );
    ...
    <div className="rounded-xl border border-stone-200 bg-white shadow-sm">
        ...
        <div className="flex gap-2">
          {messages.length > 0 && (
            <button
              type="button"
              onClick={handleClear}
              className="text-xs text-red-500 hover:text-red-600 focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 transition-colors"
            >
              クリア
            </button>
          )}
          <button
            ...
            className="text-xs text-stone-500 hover:text-stone-600 focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 transition-colors"
          >
            閉じる
          </button>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 border-t border-stone-100 px-4 py-2.5">
        <input
          ...
          placeholder="質問を入力..."
          disabled={loading}
          className="flex-1 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-800 placeholder-stone-500 focus:border-amber-400 focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 focus:outline-none transition-colors"
        />
        <button
          type="submit"
          disabled={!input.trim() || loading}
          className="rounded-lg bg-stone-800 px-4 py-2 text-sm font-medium text-white hover:bg-stone-700 disabled:opacity-40 disabled:cursor-default focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 transition-colors"
        >
          送信
        </button>
      </form>
